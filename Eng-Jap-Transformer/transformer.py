...
dataset = load_parallel_data("english.txt", "japanese.txt")
...
if os.path.exists("eng_vocab.pkl"):
    ...
    with open("jap_vocab.pkl","rb") as f:
        jap_vocab = pickle.load(f)
    print("Loaded saved vocabularies.")
else:
    eng_sentences = [e for e,_ in dataset]
    tam_sentences = [t for _,t in dataset]

    eng_vocab = build_vocab(eng_sentences)
    jap_vocab = build_vocab(tam_sentences)

    with open("eng_vocab.pkl","wb") as f:
        pickle.dump(eng_vocab,f)
    with open("jap_vocab.pkl","wb") as f:
        pickle.dump(jap_vocab,f)

    print("Built and saved vocabularies.")

inv_jap_vocab = {v:k for k,v in jap_vocab.items()}
...
train_data = [(encode_sentence(e,eng_vocab),
               encode_sentence(t,jap_vocab)) for e,t in dataset]
...
model = TransformerMT(len(eng_vocab),len(jap_vocab)).to(device)
optimizer = torch.optim.Adam(model.parameters(),lr=0.0003)
criterion = nn.CrossEntropyLoss(ignore_index=0)

if os.path.exists("eng_ta_transformer.pth"):
    model.load_state_dict(torch.load("eng_ta_transformer.pth",map_location=device))
    model.eval()
    ...
    torch.save(model.state_dict(),"eng_ta_transformer.pth")
...
def translate(sentence):
    ...
    return " ".join(inv_jap_vocab.get(i,"") for i in tgt_seq[1:-1])
